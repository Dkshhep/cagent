...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".cagent").mkdir(exist_ok=True)
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "cagent/0.1"
...
def test_openai_compatible_client_sends_prompt_cache_fields_and_records_usage():
    ...
    assert client.last_completion_metadata["uncached_input_tokens"] == 512
    assert client.last_completion_metadata["cache_hit_ratio"] == 1536 / 2048
    assert client.last_completion_metadata["cache_eligible"] is True
    assert client.last_completion_metadata["prompt_cache_key_sent"] is True
    assert client.last_completion_metadata["prompt_cache_retention_sent"] is True


def test_openai_compatible_client_accepts_prompt_cache_hit_miss_usage_fields():
    class FakeResponse:
        headers = {"Content-Type": "application/json"}
...
def test_openai_compatible_client_accepts_cache_read_usage_fields():
    class FakeResponse:
        headers = {"Content-Type": "application/json"}
...
def test_openai_compatible_client_omits_prompt_cache_fields_by_default_and_marks_short_prompt_ineligible():
    captured = {}
    ...
    assert client.last_completion_metadata["cached_tokens"] == 0
    assert client.last_completion_metadata["cache_hit"] is False
    assert client.last_completion_metadata["cache_hit_ratio"] == 0.0
    assert client.last_completion_metadata["uncached_input_tokens"] == 512
    assert client.last_completion_metadata["cache_eligible"] is False
...
def test_deepseek_compatible_client_records_cache_usage_metadata():
    class FakeResponse:
        headers = {"Content-Type": "application/json"}
        ...
        def read(self):
            return json.dumps(
                {
                    "id": "153c2b69-8c79-408d-a286-88515431d884",
                    "type": "message",
                    "role": "assistant",
                    ...
                    "content": [
                        {
                            ...
                            "signature": "153c2b69-8c79-408d-a286-88515431d884",
                        },
                        {
                            "type": "text",
                            "text": "<final>我是 **cagent**，一个运行在你本地仓库中的小型编码代理。</final>",
                        },
                    ],
    ...
    assert result == "<final>我是 **cagent**，一个运行在你本地仓库中的小型编码代理。</final>"
    assert client.last_completion_metadata["prompt_cache_supported"] is True
    assert client.last_completion_metadata["cached_tokens"] == 896
    assert client.last_completion_metadata["cache_hit"] is True
    assert client.last_completion_metadata["uncached_input_tokens"] == 148
    assert client.last_completion_metadata["cache_creation_input_tokens"] == 0
    assert client.last_completion_metadata["input_tokens"] == 1044
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "cagent.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("cagent.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "cagent.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("cagent.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("cagent.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_deepseek_provider_and_env_configuration(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "cagent.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("cagent.cli.DeepSeekCompatibleModelClient") as mock_deepseek:
...
def test_build_agent_uses_deepseek_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(os.environ, {"DEEPSEEK_API_KEY": "sk-deepseek"}, clear=True):
        with patch("cagent.cli.DeepSeekCompatibleModelClient") as mock_deepseek:
            mini_pkg.build_agent(args)
...
def test_successful_run_persists_run_artifacts_and_stop_reason(tmp_path):
    ...
    runs_root = tmp_path / ".cagent" / "runs"
...
def seed_distillable_history(agent, count=36):
    for index in range(count):
        role = "user" if index % 2 == 0 else "assistant"
        content = f"history-{index} " + ("middle detail " * 80)
        if index == 5:
            content = "Completed A by editing runtime.py " + ("detail " * 80)
        if index == 6:
            content = "pytest failed for tests/test_api.py " + ("failure " * 80)
        if index == 7:
            content = "Excluded approach D because it broke cache " + ("excluded " * 80)
        agent.record({"role": role, "content": content, "created_at": f"2026-04-07T10:{index:02d}:00+00:00"})


def configure_distillation_budget(agent):
...
def test_context_distillation_retries_invalid_json_once(tmp_path):
    distill_json = json.dumps(
        {
            "checkpoint_updates": {"completed": ["A"], "failed": [], "excluded": []},
            "process_notes": [],
        }
    )
    agent = build_agent(tmp_path, ["not-json", distill_json, "<final>Done.</final>"])
    seed_distillable_history(agent)
    configure_distillation_budget(agent)
...
def test_context_distillation_double_failure_keeps_history_and_uses_v1_prompt(tmp_path):
    agent = build_agent(tmp_path, ["not-json", "still-not-json", "<final>Done.</final>"])
    seed_distillable_history(agent)
    original_history_len = len(agent.session["history"])
    configure_distillation_budget(agent)

    assert agent.ask("continue") == "Done."

    assert len(agent.session["history"]) == original_history_len + 2
    assert not any(item.get("metadata", {}).get("kind") == "distilled_history" for item in agent.session["history"])
    trace_events = [
        json.loads(line)
        for line in agent.run_store.trace_path(agent.current_task_state).read_text(encoding="utf-8").splitlines()
    ]
...
def test_context_distillation_process_note_can_be_recalled_later(tmp_path):
    ...
    agent = build_agent(tmp_path, [distill_json, "<final>Done.</final>", "<final>Recalled.</final>"])
    seed_distillable_history(agent)
    configure_distillation_budget(agent)

    assert agent.ask("continue pytest work") == "Done."
    agent.context_manager.section_budgets["history"] = 80000
    assert agent.ask("Why did pytest fail for test_api?") == "Recalled."
    assert "pytest failed because tests/test_api.py expected the old route." in agent.model_client.prompts[-1]
...
def test_agent_records_model_cache_metadata_in_last_prompt_metadata(tmp_path):
    ...
    assert agent.model_client.complete_kwargs[-1]["prompt_cache_key"] == agent.last_prompt_metadata["prefix_hash"]
    assert agent.model_client.complete_kwargs[-1]["prompt_cache_retention"] is None
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert DeepSeekCompatibleModelClient is not None
