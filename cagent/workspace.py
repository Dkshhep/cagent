...
IGNORED_PATH_NAMES = {".git", ".cagent", "__pycache__", ".pytest_cache", ".ruff_cache", ".venv", "venv"}
...
class WorkspaceContext:
    def __init__(self, cwd, repo_root, branch, default_branch, recent_commits, project_docs):
        self.cwd = cwd
    ...
    def text(self):
        ...
        # 刻意不含 `git status`：改动文件列表每次写操作都会变，放进稳定前缀里
        # 会反复击穿 prompt 前缀缓存；当前改动状态交给 history/工具结果体现。
