...
OPENAI_COMPATIBLE_USER_AGENT = "cagent/0.1"
...
def _extract_usage_cache_details(data):
    ...
    raw_input_tokens = int(usage.get("input_tokens", usage.get("prompt_tokens")) or 0)
    if cache_miss_tokens is not None:
        uncached_input_tokens = int(cache_miss_tokens or 0)
...
class OpenAICompatibleModelClient:
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        在 agent 链路里的位置：
        它位于 `CAgent.ask()` 的模型调用阶段，是稳定前缀缓存复用链路真正
        落到 provider API 的地方。
        """
        self.last_completion_metadata = {}
        payload = {
...
def _extract_deepseek_usage_cache_details(data):
    usage = data.get("usage") or {}
    uncached_input_tokens = int(usage.get("input_tokens") or usage.get("prompt_cache_miss_tokens") or 0)
    cached_tokens = int(usage.get("cache_read_input_tokens") or usage.get("prompt_cache_hit_tokens") or 0)
    cache_creation_tokens = int(usage.get("cache_creation_input_tokens") or 0)
    output_tokens = usage.get("output_tokens")
    total_input_tokens = uncached_input_tokens + cached_tokens + cache_creation_tokens
    return {
...
class DeepSeekCompatibleModelClient:
    def __init__(self, model, base_url, api_key, temperature, timeout):
        self.model = model
        self.base_url = _normalize_versioned_base_url(base_url)
        self.api_key = api_key
    ...
    def complete(self, prompt, max_new_tokens, prompt_cache_key=None, prompt_cache_retention=None):
        ...
        for attempt in range(attempts):
            try:
                ...
                if exc.code >= 500 and attempt < attempts - 1:
                    time.sleep(0.5 * (attempt + 1))
                    continue
                raise RuntimeError(f"DeepSeek-compatible request failed with HTTP {exc.code}: {body}") from exc
            except (urllib.error.URLError, RemoteDisconnected) as exc:
                if attempt < attempts - 1:
                    time.sleep(0.5 * (attempt + 1))
                    continue
                raise RuntimeError(
                    "Could not reach the DeepSeek-compatible backend.\n"
                    f"Base URL: {self.base_url}\n"
                    f"Model: {self.model}"
                ) from exc
        ...
        try:
            ...
            raise RuntimeError(
                "DeepSeek-compatible error: backend returned non-JSON content that could not be parsed"
            ) from exc
        if data.get("error"):
            raise RuntimeError(f"DeepSeek-compatible error: {data['error']}")
        self.last_completion_metadata = {
            "prompt_cache_supported": True,
            **_extract_deepseek_usage_cache_details(data),
        }
        text = _extract_anthropic_text(data)
        if text:
            return text
