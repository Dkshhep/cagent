...
class CAgent:
    ...
    def should_distill_context(self, prompt_metadata):
        if not self.feature_enabled("context_distillation"):
            return False
        history_meta = dict(prompt_metadata.get("history", {}) or {})
        history_section = dict(prompt_metadata.get("sections", {}).get("history", {}) or {})
        history_was_compressed = int(history_section.get("estimated_tokens", 0)) < int(history_section.get("raw_tokens_estimated", 0))
        return bool((prompt_metadata.get("budget_reductions") or history_was_compressed) and int(history_meta.get("distillation_candidate_count", 0)) > 0)

    def compressed_history_candidate_transcript(self, start, end):
        history = list(self.session.get("history", []))
        candidate = history[int(start) : int(end)]
        entries = []
        for offset, item in enumerate(candidate, start=int(start)):
            lines, _ = self.context_manager._render_compressed_history_item(
                item=item,
                index=offset,
                recent=False,
                duplicate=False,
    ...
    def build_history_distillation_prompt(self, candidate_transcript):
    ...
    def parse_distillation_json(self, raw):
    ...
    def run_context_distillation(self, task_state, user_message, prompt_metadata):
        ...
        candidate_count = int(history_meta.get("distillation_candidate_count", 0))
        if start is None or end is None or candidate_count <= 0:
        ...
        for attempt in range(2):
            retry_count = attempt
            try:
                raw = self.model_client.complete(distill_prompt, DISTILLATION_MAX_NEW_TOKENS)
                parsed = self.parse_distillation_json(raw)
                break
        ...
        if parsed is None:
            ...
            return {
                "status": "failed",
                "reason": last_error or "invalid_json",
    ...
    def ask(self, user_message):
        ...
        如果新人想理解 cagent 是怎么“从一句话跑成一个 agent 流程”的，
        这里就是最关键的入口。
        """
        ...
        while tool_steps < self.max_steps and attempts < max_attempts:
            ...
            if self.should_distill_context(prompt_metadata):
                ...
                if distillation_result.get("status") == "success":
                    prompt, prompt_metadata = self._build_prompt_and_metadata(user_message)
                    prompt_metadata["context_distillation"] = dict(distillation_result)
            if self.pre_prompt is not None:
                common, uncommon_pre_prompt, uncommon_prompt = common_prefix_split(self.pre_prompt, prompt)
            ...
            if self.feature_enabled("prompt_cache") and bool(getattr(self.model_client, "supports_prompt_cache", False)):
                prompt_cache_key = prompt_metadata.get("prompt_cache_key")
            ...
            """
            对模型后端的调用很简单：丢 prompt 进去，拿文本出来。prompt cache 的 key 是前缀 hash——因为 tools + workspace 部分在大多数轮次中是不变的，后端可以复用计算。
            注意：supports_prompt_cache 只在 openai.com 和 right.codes 后端为 True，其他后端传了也会被忽略（models.py:235）。
            """
    ...
    def parse(raw):
        ...
        if "<tool>" in raw and ("<final>" not in raw or raw.find("<tool>") < raw.find("<final>")):
            ...
            try:
                ...
                return "retry", CAgent.retry_notice("model returned malformed tool JSON")
            if not isinstance(payload, dict):
                return "retry", CAgent.retry_notice("tool payload must be a JSON object")
            if not str(payload.get("name", "")).strip():
                return "retry", CAgent.retry_notice("tool payload is missing a tool name")
            args = payload.get("args", {})
            if args is None:
                payload["args"] = {}
            elif not isinstance(args, dict):
                return "retry", CAgent.retry_notice()
            return "tool", payload
        if "<tool" in raw and ("<final>" not in raw or raw.find("<tool") < raw.find("<final>")):
            payload = CAgent.parse_xml_tool(raw)
        ...
        if "<final>" in raw:
            ...
            if final:
                return "final", final
            return "retry", CAgent.retry_notice("model returned an empty <final> answer")
        raw = raw.strip()
        if raw:
            return "final", raw
    ...
    def parse_xml_tool(raw):
        ...
        if not match:
            return None
        attrs = CAgent.parse_attrs(match.group("attrs"))
        ...
        for key in ("content", "old_text", "new_text", "command", "task", "pattern", "path"):
            if f"<{key}>" in body:
                args[key] = CAgent.extract_raw(body, key)
